...
namespace RocketShop.Warehouse.Admin.Repository
{
    ...
    {
        public async Task<FlexibleDataReport> CallData(string collectionName,string? orderBy,bool asc,int?page,int? per,NpgsqlConnection warehouseConnection)
        {
        ...
        public async Task<FlexibleDataReport> CallDataWithCondition(string collectionName,string where, string? orderBy, bool asc, int? page, int? per, NpgsqlConnection warehouseConnection)
        {
            ...
            string query = $@"select * from ""{collectionName}"" where {where}";
            if (page.HasValue)
            {
                query += " limit @limit offset @offset";
                var offsetParam = cmd.CreateParameter();
            ...
            cmd.Parameters.Clear();
            cmd.CommandText = $"select count(*) from \"{collectionName}\" where {where}";
            returnValue.TotalCount = (long)(await cmd.ExecuteScalarAsync())!;
